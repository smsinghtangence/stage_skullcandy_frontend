...
function Review({ product }) {
   ...
   const [formData, setFormData] = useState({
      name: users?.firstname ? users?.firstname : '',
   ...
   const getdata = async () => {
      ...
      const response = await getDataWithQuery(`/api/ratings?filters[product_id][$eq]=${product_id}&filters[approved][$eq]=true`,
         {
         });
      setData(response?.data)
      const found = response?.data?.filter(res => res?.attributes?.user_id == users?.id);

       if(found?.length>0){
         console.log("found inside")
         setAllowedReivew(false);
       }
   else
    setAllowedReivew(true)
   ...
   const getUserPurchased = async () => {
      ...
      const response = await axios.get(API_URL + `/api/orders?filters[customer_id][$eq]=${users?.id}&filters[line_items][product_id][$eq]=${product_id}&populate=*`, {
         headers: {
   ...
   useEffect(() => {
      ...
      if (product_id != undefined)
         getdata();
      if (product_id != undefined && users?.id)
         getUserPurchased();
...
useEffect(() => {
   ...
   const handleChange = (e) => {
      ...
      //  const product_id = product?.id
      setFormData({ ...formData, [name]: value, product_name: product?.title, product_id: `${product_id}`, user_id: `${users?.id}` },);
      // setFormData( {
            ...
            <div className="container">
               ...
               <nav aria-label="Page navigation example">
                  <ul className="pagination justify-content-center">
                     ...
                     {/* review form and btn start */}
                     {/* user if login */}
                     
                     {users?.id &&  Userpurchased && AllowedReivew &&
                        <div className="review-form pt-5 pb-5">
                           <div className="container">
                              <div className="row">
                                 <div className="col-lg-12">
